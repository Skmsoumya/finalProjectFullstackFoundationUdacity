...
def fbconnect():
	...
	url = "https://graph.facebook.com/oauth/access_token?grant_type=fb_exchange_token&client_id=%s&client_secret=%s&fb_access_token=%s" % (app_id, app_secret, access_token)
	h = httplib2.Http()
	...
	#get user picture
	url = "http://graph.facebook.com/v2.2/me/picture?%s&redirect=0&height=200&width=200" % token
	h = httplib2.Http()
	result = h.request(url, "GET")[1]
	data = json.loads(result)
